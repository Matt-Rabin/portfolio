import { c as createComponent } from './astro-component_D1Ilb5b_.mjs';
import 'piccolore';
import { i as renderComponent, r as renderTemplate, m as maybeRenderHead, f as addAttribute, j as Fragment } from './ssr-function_Kw3I_afp.mjs';
import { $ as $$SaleLayout } from './SaleLayout_C7x-_vW-.mjs';
import { g as getSaleConfig, a as getSaleAdminUser } from './supabase_CgPIPyeq.mjs';
import { g as getAdminSnapshot } from './repository_CI7S-sa6.mjs';
...
const $$Admin = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const adminUser = config.routeSlug === "mattslist" ? await getSaleAdminUser(Astro2) : null;
  const notice = Astro2.url.searchParams.get("notice");
  const signInSent = Astro2.url.searchParams.get("sent") === "1";
...
</p> <form class="sale-form-grid" method="post"${addAttribute(`/api/sale/${config.routeSlug}/auth/sign-in`, "action")}> <label class="sale-field"> <span class="sale-label">Admin email</span> <input name="email" type="email" autocomplete="email" required${addAttribute(config.adminEmail, "placeholder")}> </label> <button class="sale-button sale-button-primary" type="submit">Send magic link</button> </form> </section>`} </div>` : renderTemplate`<div class="sale-admin-page"> <section class="sale-panel sale-hero"> <h1 class="sale-headline">Not found.</h1> <p class="sale-copy">This admin route only exists for the configured hidden sale slug.</p> </section> </div>`}` })}`;
