...
import 'piccolore';
import { i as renderComponent, r as renderTemplate, m as maybeRenderHead, f as addAttribute, j as Fragment } from './ssr-function_9gJIHixD.mjs';
import { $ as $$SaleLayout } from './SaleLayout_XORbBpGt.mjs';
...
const $$Admin = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const errorDetail = Astro2.url.searchParams.get("error");
...
</p> <details class="sale-panel" style="padding: 1rem; margin-bottom: 1rem;"> <summary class="sale-card-title" style="cursor: pointer;">Debug info</summary> <div class="sale-copy" style="margin-top: 0.75rem;"> <div><strong>Current URL:</strong> ${Astro2.url.pathname}</div> <div><strong>Configured slug:</strong> ${config.routeSlug}</div> <div><strong>Admin path:</strong> ${adminPath}</div> <div><strong>Sign-in path:</strong> ${signInPath}</div> <div><strong>Sign-out path:</strong> ${signOutPath}</div> <div><strong>Notice:</strong> ${notice ?? "none"}</div> <div><strong>Error detail:</strong> ${errorDetail ?? "none"}</div> </div> </details> <form class="sale-form-grid" method="post"${addAttribute(signInPath, "action")}> <input type="hidden" name="next"${addAttribute(adminPath, "value")}> <label class="sale-field"> <span class="sale-label">Admin password</span> <input name="password" type="password" autocomplete="current-password" required> </label> <button class="sale-button sale-button-primary" type="submit">Sign in</button> </form> </section>`} </div>` : renderTemplate`<div class="sale-admin-page"> <section class="sale-panel sale-hero"> <h1 class="sale-headline">Not found.</h1> <p class="sale-copy">This admin route only exists for the configured hidden sale slug.</p> </section> </div>`}` })}`;
